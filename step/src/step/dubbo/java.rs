...
impl StepRunner for Dubbo {
    async fn run(&self, arg: &dyn RunArg) -> StepValue {
        let registry_address = arg.config()["registry_address"]
            .as_str()
        ...
        let parts = method_long.split(&['#', '(', ',', ')'][..]).collect::<Vec<&str>>();
        if parts.len() < 2 {
        ...
        let invoke = GenericInvoke {
            reference: Reference {
                ...
                registry_address: registry_address.to_owned()
            },
...
pub async fn create(config: Option<&Json>, _: &dyn CreateArg) -> Result<Box<dyn StepRunner>, Error> {
    if config.is_none() {
        return rerr!("010", "missing config");
    }
    let conf = config.unwrap();

    let jar_path = conf["jar_path"]
        .as_str()
    ...
    child.stdout = None;
    Ok(Box::new(Dubbo { child, port}))
}


async fn remote_invoke(port: usize, remote_arg: GenericInvoke) -> Result<Json, Rae>{
    let rb = RequestBuilder::new(Method::Post, Url::from_str(format!("http://127.0.0.1:{}/invoke", port).as_str()).or(rerr!("021", "invalid url"))?);
