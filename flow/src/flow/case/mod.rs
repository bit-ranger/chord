...
use chord_common::point::{PointAssess, PointState, PointArg};
pub mod res;
pub mod arg;
...
pub async fn run(flow_ctx: &dyn FlowContext, arg: &CaseArgStruct) -> CaseAssessStruct {
    ...
    for (point_id, point_runner) in arg.point_runner_vec() {
        ...
        match point_assess{
            PointAssessStruct {state: PointState::Err(e), id,start, end:_} => {
                let config_rendered = render(flow_ctx.get_handlebars(), &render_context, config_raw.as_str()).unwrap_or("".to_owned());
            ...
            PointAssessStruct {state: PointState::Fail(json), id,start, end} => {
                let config_rendered = render(flow_ctx.get_handlebars(), &render_context, config_raw.as_str()).unwrap_or("".to_owned());
